...
int nms(detection_t* detections, int32_t num_detections,
    ...
    *output_count = 0;
    ...
    // Greedy NMS: for each detection, suppress overlapping ones
    for (int i = 0; i < num_detections && keep_count < max_detections; i++) {
        if (keep[i] == -1) continue;  // Already suppressed
        
        keep[i] = 1;
        keep_count++;
        
        // Suppress overlapping detections with same class
        for (int j = i + 1; j < num_detections; j++) {
            if (keep[j] == -1) continue;  // Already suppressed
